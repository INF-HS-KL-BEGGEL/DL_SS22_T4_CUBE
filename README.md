...
Der Spieler hat muss zudem die Figur so rotieren, dass diese in die entsprechende Form der Würfelseite passt.
Sind alle Figuren durch den Spieler in dern Würfel gelangt ist das Spiel beendet.
...
Mithilfe eines Reinforcment Learning Agenten kann dieser in Rolle des Spielers das Spiel bedienen
...
### QTable
Bei der Q-Table handelt es sich um ein Tabelle mit allen in der Umgebung möglichen Zuständen, die mithilfe
...
dann kann er unter den möglichen Aktionen die mit dem vielversprechensten Reward wählen.
Vorraussetzung für das Erlernen des Spiels ist es, dass sich die Umgebung nicht nach jeder Runde ändert
...
Das Netzwerk erlernt wie der Q-Table Agent die Umgebung kennen und gibt warscheinlichkeiten zurück,
welche Aktion für welchen Zustand am sinnvollsten erscheinen. 
Dieser Art von Agent hat die im vergleich zu Q-Table Agenten den Vorteil in größereb Zustandräumen, 
...
Für die Umsetzung des Projekt wurde die Programmiersprache Python in der Verion 3.6 verwendet. Darauf aufbauend wurden die Pakete 
Tenforflow (Version ) und matplotlib (Versiob) verwendet.
...
Docker ist eines der gängiges Tool zur Container-Virtualisierung.
...
nach außen gemappt werden. Volumens ermöglichen es Dateipfade von der Hostumgebung 
...
Einmal eine Implementierung mithilfe von QTable-Learning und eine Mit QNetwork-Learning. Die Agenten können beliebig
...
## Agenten
Innerhalb des Projekt sind zwei Implementierungen von Agenten realisiert worden. Die Basisklasse erfordert eine train und play Methode
für die konkreten Implementierungen. 
Die Implementierungen leiten von der Basis-Klasse AgentBase ab und erben somit weitere Funktionalitäten. 
Hier können sich mithilfe eines Observer-Patterns Writer Objekte registieren.
Diese leiten die Daten an entsprechende Ziele weiter. 
Aktuell wird das genutzt um die Daten entweder mithilfe einer Plotters anzuzeigen oder 
in einem Server-Modus in eine CSV-Datei wegzuschreiben.

TODO Abbildung UML

### QTableAgent
Mit Hilfe von QTable Learning kann der Agent für alle Zustände der Umgebung und den Verfügbaren Aktionen 
...
TODO Infos zu Parameter
TODO Infos zu Netz/Model
TODO Infos zu historie und unterschie zu Optimized
...
welche die konkreten Actions-Spaces und Oberservation-Spaces definieren.
...
TODO Es muss immer ein guten Ausgleich zwischen Exploration und Lernrate geben
TODO GPU Tensorflow macht Probleme
TODO Performance
    predict ist für einzelen aktionen nicht gut siehe Tensorflow doku. Eine Alternative ist der direkte call auf dem Modell
    das retrain muss nicht in jedem Step gemacht werden wenn Tensorflow die ganzen Steps in der History bekommt
...
TODO Architektur der Anwendung. Agenten und Algorithmen können auf unterschiedliche Umgebungen losgelassen werden
    ...
    TODO Unittests die nicht exitieren auch
    TODO Actionspace und Observationspace sind zu Konkret vom Spiel abhängig um diese zu veralgemeinern
